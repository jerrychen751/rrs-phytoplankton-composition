import numpy as np
from core.model import rrsModelTrain
...
def train_model(RrsD: Union[np.ndarray, pd.DataFrame], hplc: np.ndarray) -> None:
    ...
    shape [n_wl, 100]) and C (intercepts, shape [100]) to CSV files in data/model_coefficients/.
    """
    ...
    # Create model_coefficients directory if it doesn't exist
    project_root = get_project_root()
    output_dir = project_root / 'data' / 'model_coefficients'
    ...
    for i in range(len(pigs2mdl)):
        ...
        # save coefficients to CSV files in model_coefficients folder
