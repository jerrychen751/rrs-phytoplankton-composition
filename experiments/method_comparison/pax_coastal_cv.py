...
without the domain-shift confound of the previous Kramer → PAX experiment.
...
7 methods compared (4 linear + 3 nonlinear):
  - SDP (Kramer et al. — 100-permutation PCA + linear regression ensemble)
  - PLS (supervised dimensionality reduction)
  - ElasticNet (L1+L2 regularized linear)
  - Ridge (L2-regularized linear, no PCA)
  - HistGBT (gradient boosted trees)
  - KernelRidge (RBF kernel ridge regression)
  - SVR (RBF support vector regression)

Usage:
    python experiments/method_comparison/pax_coastal_cv.py

Outputs: experiments/method_comparison/outputs/pax_coastal_cv/
...
# --- Limit BLAS/LAPACK thread count BEFORE importing numpy ---
...
# benefit. Without this cap, NumPy's SVD spawns 6-8 threads per call.
...
from experiments.method_comparison.config import CONFIG
...
def run_all_cv(
    ...
    """
    pigments = config["data"]["pigments"]
    all_methods = config["sklearn"]["methods"]
...
METHOD_COLORS = {
    "SDP": "#4C72B0",         # steel blue (the baseline)
    "PLS": "#DD8452",         # muted orange
    "ElasticNet": "#55A868",  # sage green
    "HistGBT": "#C44E52",     # muted red
    "Ridge": "#CCB974",       # muted gold
    "KernelRidge": "#64B5CD", # sky blue
    "SVR": "#8172B3",         # muted purple
}
...
def main() -> None:
    ...
    print(f"Starting LOOCV with {n_matchups} samples, {n_methods} methods, {len(CONFIG['data']['pigments'])} pigments")
    print(f"{'=' * 60}")
